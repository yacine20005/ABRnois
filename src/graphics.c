...
void generate_pdf(char *filename, ABRnois tree)
{
    char dot_path[100];
    char pdf_path[100];
    char cmd[100];

    snprintf(dot_path, sizeof(dot_path), "data/%s.dot", filename);
    snprintf(pdf_path, sizeof(pdf_path), "data/%s.pdf", filename);

    FILE *out = fopen(dot_path, "w");
    generate_dot(out, tree);
    fclose(out);

    int len = strlen(dot_path) + strlen(pdf_path) + 15;
